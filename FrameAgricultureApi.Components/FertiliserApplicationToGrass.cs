...
/// <summary>
/// Class containg calculation for fertiliser applied to grass
/// </summary>
public static class FertiliserApplicationToGrass
{
    /// <summary>
    ...
    /// <param name="sownWithClover">boolean indicating grass sown with clover</param>
    /// <param name="receivesManagedManure">boolean indicating if grass receives managed manure</param>
    ...
    public static FertiliserEmissions UnmitigatedEmissionsFertiliser(FertiliserType fertiliserType, double[] FertiliserApplications, double totalNapplied, double percentageFertiliserType, int gridSquare_ID, int soilType, int grassType, int grassUseType, bool sownWithClover, bool receivesManagedManure, bool acidicSoil)
    {
        ...
        if(PhysicalLookup.IsValidGridSquare(gridSquare_ID))
        {
            ...
            //Calculate frac leach for grass
            int climateregion = GrassLookup.Instance.ClimateRegion(gridSquare_ID);
            double fracleach = 0.0;
            GrassEquationCoefficients coeff = GrassLookup.Instance.GrassFracLeachCoefficients(climateregion, soilType, grassType, grassUseType, sownWithClover, receivesManagedManure);

            fracleach = coeff.C + (coeff.Coeff_X * rateN) + (coeff.Coeff_X2 * Math.Pow(rateN, 2)) + (coeff.Coeff_X3 * Math.Pow(rateN, 3)) + (coeff.Coeff_X4 * Math.Pow(rateN, 4));

            //Calculate NO3-N emissions
            unmitigatedEmissions.EmissionsCore[CoreEmissions.LeachedNO3N].Value = FertiliserEquations.NO3N_Leached(rateN, fracleach);
    ...
    public static FertiliserEmissions MitigatedEmissionsFertiliser(FertiliserType fertiliserType, double[] FertiliserApplications, double totalNapplied, double percentageFertiliserType, int gridSquare_ID, int soilType, int grassType, int grassUseType, bool sownWithClover, bool receivesManagedManure, bool acidicSoil, List<int> mitigationMethods)
    {
                ...
                (int)OrganicMatterType.NotSet, out MitigationFactor multiplier, out string mitigationErrors))
        {
            ...
            if(PhysicalLookup.IsValidGridSquare(gridSquare_ID))
            {
                ...
                //Calculate NO3-N emissions
                mitigatedEmissions.EmissionsCore[CoreEmissions.LeachedNO3N].Value = FertiliserEquations.NO3N_Leached(rateN, fracleach);
